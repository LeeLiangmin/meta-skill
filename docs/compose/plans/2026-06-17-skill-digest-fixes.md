...
**Goal:** 根据 skill-evaluator 评估报告修复 skill-digest 的 2 个高严重度和 4 个中等问题（F1-F8），改动集中在 `skill-digest/SKILL.md` 单文件。
...
**Files:**
- Modify: `skill-digest/SKILL.md:L3`
...
Run: `grep "触发词区分" skill-digest/SKILL.md`
Expected: 输出包含该行

- [ ] **Step 3: Commit**

```bash
git add skill-digest/SKILL.md
...
**Files:**
- Modify: `skill-digest/SKILL.md:L14-L34`
...
Run: `grep "本 skill 自身的类型" skill-digest/SKILL.md && grep "判为混合型" skill-digest/SKILL.md`
Expected: 两行均存在
...
**Files:**
- Modify: `skill-digest/SKILL.md:L84-L97`
...
Run: `grep "← 所有类型适用" skill-digest/SKILL.md`
Expected: 2 行匹配
...
**Files:**
- Modify: `skill-digest/SKILL.md:L107`
...
Run: `grep "默认将简报直接在对话中展示" skill-digest/SKILL.md`
Expected: 输出包含该行
...
**Files:**
- Modify: `skill-digest/SKILL.md:L48-L49`（第 5 步输出步骤）
...
Run: `grep "交付后不要在对话中附加" skill-digest/SKILL.md`
Expected: 输出包含该行
...
**Files:**
- Read: `skill-digest-eval/report.md`（旧报告，对照用）
