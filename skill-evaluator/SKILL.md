...
description: 评估/审计一个已有 skill，输出基于事实的测试报告。每条结论必须可追溯到证据。触发词：测评/评估/审计 skill、skill 跑得怎么样、出测试报告、核对运行记录。不用于创建新 skill（那是 skill-creator 的职责）。
license: MIT
...
**校验首选 subagent**：给 subagent **只**提供原始来源片段和一个精确的是非问题，**不要**把你的结论告诉它——独立性正是校验的价值所在。

- **OpenCode**：通过 `task` 工具派发校验任务给专用 subagent（项目内的 `agents/evaluator-verifier.md`，install.sh 安装到 `.opencode/agents/`。只读、禁 bash）。child session 完成后其产出可在 parent session 读取。
- **Claude Code / MiMoCode**：使用 `actor` 工具 spawn 独立 subagent，只给原始材料和非问题。

subagent 返回 confirmed / refuted / inconclusive 及它自己找到的证据，记回账本。**没有 subagent 时**退化为"新视角复核"：从零重新打开原始来源，忽略自己之前的笔记，重新独立回答那个精确问题——这比独立校验弱，要在报告方法学里如实说明。

### 第 4 步：写发现，机检，再渲染报告

把每条结论写进 `findings.json`：每个 finding 必须带至少一个 `evidence_ids`；inferred 的必须有 `reasoning`；高严重度的必须有 `verification`。然后**先机检再出报告**：

脚本位于本 skill 的 `scripts/` 目录下。执行前先用 glob 或 list 工具定位 skill-evaluator 的安装路径（可能在 `skill-evaluator/`、`.opencode/skills/skill-evaluator/`、`~/.config/opencode/skills/skill-evaluator/` 或 `.claude/skills/skill-evaluator/` 中），然后用绝对路径调用。示例（设 `$EVAL_DIR` 为 skill-evaluator 安装目录）：

```bash
# Linux/macOS
python3 $EVAL_DIR/scripts/check_report.py <skill-name>-eval/evidence.json <skill-name>-eval/findings.json

# Windows
python $EVAL_DIR\scripts\check_report.py <skill-name>-eval\evidence.json <skill-name>-eval\findings.json
```
...
```bash
# Linux/macOS
python3 $EVAL_DIR/scripts/render_report.py <skill-name>-eval/evidence.json <skill-name>-eval/findings.json -o <skill-name>-eval/report.md

# Windows
python $EVAL_DIR\scripts\render_report.py <skill-name>-eval\evidence.json <skill-name>-eval\findings.json -o <skill-name>-eval\report.md
```
...
```bash
# Linux/macOS
python3 $EVAL_DIR/scripts/check_citations.py <skill-name>-eval/最终报告.md <skill-name>-eval/evidence.json

# Windows
python $EVAL_DIR\scripts\check_citations.py <skill-name>-eval\最终报告.md <skill-name>-eval\evidence.json
```
```
