...
- `run-results/<timestamp>/summary.json` —— 获取 `results` 数组，含每个用例的 `output_file` 路径和 `status`
...
读取 `run-results/<timestamp>/<output_file>` 获取 subagent 的完整执行记录。这将成为 skill-evaluator 的 session 来源。

#### 2.2 加载 skill-evaluator 做评估

加载被测 skill `.opencode/skills/skill-evaluator/SKILL.md`（即 skill-evaluator 本身），然后给出如下指令：

> 请评估被测 skill `<target_skill>`。
>
> 我们提供一份执行 session：`run-results/<timestamp>/<output_file>`（已在上文读取）。
...
将所有用例的判定结果汇总为 `report.md`，写入 `run-results/<timestamp>/` 目录下。
...
|------|------|------|------|----------------|-------------------|----------|
| TC-001 | <名称> | <场景类型> | PASS / FAIL / SKIP | 8/10 | 1/2 | 注记 |
| ... | | | | | | |
...
skill-evaluator 在评估过程中会在 `<skill-name>-eval/` 下生成 `evidence.json` 和 `findings.json`。这些是中间产物，judge 完成后可以保留（便于追溯），也可以清理。默认**保留**——它们本身就是判定证据链的一部分。

## 关键约束

- **评估引擎必须是 skill-evaluator。** 不自编评估逻辑。自编的判断没有证据账本，不可信。
- **逐条核对 expected_behaviors。** 不跳过、不合并且"看起来类似"的条目。
- **按 severity 分级判定。** must→PASS/FAIL，should→PASS/WARN，may→仅观察。不混用、不添加中间状态。
- **每条判定必须有证据引用。** 引用格式为 `源:定位`，无引用 = 不可信 = 不应出现在报告里。
- **skill-evaluator 的 evidence.json + findings.json 是中间产物。** judge 自己不做无证据断言，必须通过 evaluator 产出。
- **SKIP 的用例不加载 skill-evaluator。** 执行失败/超时的用例没有可评估的输出。
